...
void main( void )
{
  ...
  //float n = snoise(time);
  //float n = (noise(vec4(1.0 * v_texCoord3D.xyz, 0.1 * time)) + noise(vec4(2.0 * v_texCoord3D.xyz, 0.1 * time)) + noise(vec4(4.0 * v_texCoord3D.xyz, 0.1 * time)))/3.0;

  //gl_FragColor = v_color * vec4(0.5 + 0.5 * vec3(n, n, n), 1.0);

  gl_FragColor=vec4(
  						snoise(vec4(1.0 * v_texCoord3D.xyz, 0.5 * time)       ) * 0.5 + 0.5,
  						snoise(vec4(1.0 * v_texCoord3D.xyz, 0.5 * time) + 3.33) * 0.5 + 0.5,
  						snoise(vec4(1.0 * v_texCoord3D.xyz, 0.5 * time) + 7.77) * 0.5 + 0.5,
  						1.0
  					);
}
